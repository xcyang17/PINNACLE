...
MAX_EPOCHS=${2} # 250
SAVE_PREFIX="/scratch/gilbreth/yang1641/exome/results/pinnacle/my_disease_ppi_task/epoch"${MAX_EPOCHS}
output_filename=${3}
mkdir -p ${SAVE_PREFIX}
...
if [[ -n "$LAST_MODEL" ]]; then
    ...
    python pinnacle/train.py \
        ...
        --ppi_dir "${input_dir}/my_networks/ppi_edgelists/" \
        --mg_f "${input_dir}/my_networks/phecode_phecode_edge_list_33.txt" \
        --batch_size=8 \
        --dropout=0.6 \
        --feat_mat=1024 \
        --hidden=64 \
