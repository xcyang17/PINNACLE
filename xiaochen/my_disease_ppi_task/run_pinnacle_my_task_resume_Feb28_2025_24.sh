...
MAX_EPOCHS=${2} # 250
SAVE_PREFIX="/scratch/gilbreth/yang1641/exome/results/pinnacle/my_disease_ppi_task_24/epoch"${MAX_EPOCHS}
output_filename=${3}
mkdir -p ${SAVE_PREFIX}
...
if [[ -n "$LAST_MODEL" ]]; then
    ...
    python pinnacle/train.py \
        --G_f "${input_dir}/my_networks/string_otar_biogrid_hippie_huri_menche_and_more_all_merged_ppi_human_proteins.txt" \
        --ppi_dir "${input_dir}/my_networks/ppi_edgelists_phecode_description_24/" \
        --mg_f "${input_dir}/my_networks/phecode_description_phecode_description_edge_list_24.txt" \
        --batch_size=8 \
        --dropout=0.6 \
        --feat_mat=1024 \
        --hidden=64 \
