...
### Mar 14, 2025: now adds an additional argument to control whether the link prediction on the metagraph is part of the loss function
...
MAX_EPOCHS=${2} # 250
SAVE_PREFIX="/scratch/gilbreth/yang1641/exome/results/pinnacle/reproduce_include_mg_link_pred/epoch"${MAX_EPOCHS}
output_filename=${3}
...
if [ "$EPOCHS_DONE" -lt "$MAX_EPOCHS" ]; then
    if [ "$job_i" -lt "$n_jobs" ]; then
        ...
        sbatch -N 1 --gres=gpu:2 --job-name=${wandb_project_name} --mem 40g -t 4:00:00 --mail-type=end,fail --mail-user=yang1641@purdue.edu -A standby "$0" "${nepochs}" "${MAX_EPOCHS}" "${output_filename}"
    else
